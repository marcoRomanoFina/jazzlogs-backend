-- The archive links a track editorial straight into the album editorial page
-- (anchored at that track's section), which needs the containing album's id —
-- context_name alone (just the album's NAME) isn't enough to build that link.
-- Appending at the end, so CREATE OR REPLACE VIEW is safe here (unlike V12,
-- which had to drop a column from the middle).
