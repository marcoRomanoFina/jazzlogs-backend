...
public interface ArtistRepository extends JpaRepository<Artist, UUID>, SavedItemResolver, CatalogEntityResolver {
    ...
    // EXACT/PREFIX/CONTAINS take priority over FUZZY (the `%` operator, using
    // pg_trgm's default 0.3 similarity threshold) — matchType and the final
    // ordering come out of the same CASE so they can never disagree. LIKE's
    // %/_ aren't escaped in the raw query text — a literal % or _ the user
    // typed can make PREFIX/CONTAINS behave oddly, though FUZZY still catches
